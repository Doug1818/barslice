class UserRegistrationsController < Devise::RegistrationsController
  ...
  def create
    ...
    @user.reservations.last.date = Date.strptime(params[:user][:reservations_attributes].values[0][:date],"%m/%d/%Y").to_date #if params[:reservation][:date] != ""
    ...
    if @user.save
      ...
      #BarMailer.resrequest(@bar).deliver
      #BarMailer.resrequest_confirmation(@user).deliver
    else
      render 'new'
    end
  end
